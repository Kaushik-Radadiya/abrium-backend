...
export function evaluateGoPlusRisk(risk: GoPlusRisk): RiskEvaluation {
  ...
  const criticalFlagCount = assessment.criticalFlags.length;
  ...
  if (criticalFlagCount > 0) {
    return {
      ...
      alertMessage:
        ...
        `Found ${criticalFlagCount} critical risk flags.`,
    };
  }
  ...
  if (assessment.warningFlags.length > 0) {
    return {
      ...
      alertMessage: assessment.reasons[0] ?? 'Warning risk flags found.',
    };
  }
